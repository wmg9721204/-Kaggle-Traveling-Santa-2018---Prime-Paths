...
while i<=N_c    
    ...
    writedlm(Path_File*"_Progress_49_$(i)_of_$(N_c).txt",0)
    if i!=1
        rm(Path_File*"_Progress_49_$(i-1)_of_$(N_c).txt")
    end
    ...
    if L_n<L_o
        Improve = L_o-L_n
        writedlm("Temp//Path_49.txt", [["Start", a, "Improved_Length", Improve, "Path"]; GoodPath[a:b][Path_a]])
    end
end
